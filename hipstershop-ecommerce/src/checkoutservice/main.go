...
func main() {
	...
	zbclient, err := getZBClient()

	log.Infof("zbclient: %+v, serviceEndpoint: %s, error: %v, zbEnabled %v", zbclient, zbclient.ServiceEndPoint, err, zbioEnabled)
...
func (cs *checkoutService) PlaceOrder(ctx context.Context, req *pb.PlaceOrderRequest) (*pb.PlaceOrderResponse, error) {
	log.Infof(fmt.Sprintf("[PlaceOrder] user_id=%q user_currency=%q", req.UserId, req.UserCurrency))
	var zbMessages []zb.Message
	zbMessages = append(zbMessages, zb.Message{
		TopicName:     topicName,
		Data:          []byte(fmt.Sprintf("[PlaceOrder] user_id=%q user_currency=%q", req.UserId, req.UserCurrency)),
		HintPartition: "",
	})
	...
	if err != nil {
		zbMessages = append(zbMessages, zb.Message{
			TopicName:     topicName,
			Data:          []byte(fmt.Sprintf("failed to generate orderId, ErrorCode: %v, error: %v", codes.Internal, err)),
			HintPartition: "",
		})
	...
	if err != nil {
		zbMessages = append(zbMessages, zb.Message{
			TopicName:     topicName,
			Data:          []byte(fmt.Sprintf("failed to charge card:, ErrorCode: %v, error: %v", codes.Internal, err)),
			HintPartition: "",
		})
	...
	zbMessages = append(zbMessages, zb.Message{
		TopicName:     topicName,
		Data:          []byte(fmt.Sprintf("Card charged successfully, Transaction Id: %v", txID)),
		HintPartition: "",
	})
	...
	if err != nil {
		zbMessages = append(zbMessages, zb.Message{
			TopicName:     topicName,
			Data:          []byte(fmt.Sprintf("shipping error: %+v, ErrorCode: %v", err, codes.Unavailable)),
			HintPartition: "",
		})
	...
	zbMessages = append(zbMessages, zb.Message{
		TopicName:     topicName,
		Data:          []byte(fmt.Sprintf("shippingTrackingID : %v", shippingTrackingID)),
		HintPartition: "",
	})
	...
	if err := cs.sendOrderConfirmation(ctx, req.Email, orderResult); err != nil {
		zbMessages = append(zbMessages, zb.Message{
			TopicName: topicName,
			Data: []byte(fmt.Sprintf("failed to send order confirmation. Order Id %s, Tracking Id: %s, Shipping Cost: %v, Shipping Address %v, Items count %d:",
				orderResult.GetOrderId(), orderResult.GetShippingTrackingId(), orderResult.GetShippingCost(), orderResult.GetShippingAddress(), len(orderResult.GetItems()))),
			HintPartition: "",
		})
		sendMessageToZBIO(zbMessages)
		...
		zbMessages = append(zbMessages, zb.Message{
			TopicName: topicName,
			Data: []byte(fmt.Sprintf("Order Id %s, Tracking Id: %s, Shipping Cost: %v, Shipping Address %v, Items count %d:, ErrorCode: %v, error: %v",
				orderResult.GetOrderId(), orderResult.GetShippingTrackingId(), orderResult.GetShippingCost(), orderResult.GetShippingAddress(), len(orderResult.GetItems()), codes.Unavailable, err)),
			HintPartition: "",
		})
	...
	zbMessages = append(zbMessages, zb.Message{
		TopicName: topicName,
		Data: []byte(fmt.Sprintf("Order placed. Order Id %s, Tracking Id: %s, Shipping Cost: %v, Shipping Address %v, Items count %d:, ErrorCode: %v, error: %v",
			orderResult.GetOrderId(), orderResult.GetShippingTrackingId(), orderResult.GetShippingCost(), orderResult.GetShippingAddress(), len(orderResult.GetItems()), codes.Unavailable, err)),
		HintPartition: "",
	})
